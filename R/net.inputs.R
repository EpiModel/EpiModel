...
#' @param start For models with network resimulation, time step to start up
...
#' @param tergmLite Logical indicating usage of either full network
#'        (\code{tergmLite = FALSE}), or sparse network
#'        (\code{tergmLite = TRUE}) representation.
...
control.net <- function(type,
  ...
  if ("save.network" %in% names(dot.args) ||
      "save.transmat" %in% names(dot.args)) {
    p$tergmLite <- FALSE
    stop("EpiModel 2.0+ has integrated options for saving of the network object
         and transmission matrix into control.net setting tergmLite: if FALSE,
         these are saved automatically; if TRUE, they are not saved. Update
  ...
  if (length(bi.nms) > 0) {
    ...
    if (!is.null(p$type) && sum(flag1, na.rm = TRUE) != length(flag1)) {
      stop("Control parameter 'type' must be null if any user defined
  ...
  if (!is.null(p$type) && length(p$user.mods) > 0) {
    stop("Control parameter 'type' must be null if any user specified
         modules are present",
         call. = FALSE)
  }
...
crosscheck.net <- function(x, param, init, control) {
    ...
    if (control$start == 1 && control$skip.check == FALSE) {
      ...
      if (statOnNw == TRUE) {
        ...
        if ("i.num" %in% names(init) && nw1 != init1) {
          warning("Overriding init settings with network status attribute",
      ...
      if (!is.null(init$status.vector)) {
        ...
        if (control$type == "SIR") {
          if (any(svals %in% c("s", "i", "r") == FALSE)) {
            stop("status.vector contains values other than
                 \"s\", \"i\", and \"r\" ",
                 call. = FALSE)
          }
        } else {
    ...
    if (control$start > 1) {
      ...
      if (control$skip.check == FALSE) {
        ...
        if (control$start > x$control$nsteps + 1) {
          stop("control setting start must be 1 greater than the
               nsteps in the prior simulation", call. = FALSE)
        }
  ...
  if (!is.null(control$type) && length(control$user.mods) > 0) {
    stop("Control setting 'type' must be NULL if any user-specified
         modules specified.",
         call. = FALSE)
  }
