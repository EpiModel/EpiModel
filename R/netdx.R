...
netdx <- function(x, nsims = 1, dynamic = TRUE, nsteps = NULL,
  ...
  if (inherits(x, "netclique")) {
    stop("netdx() does not apply to a `netclique` layer: its edges are fixed ",
         "by construction, so there is no model to diagnose. Print the ",
  ...
  ncores <- if (nsims == 1) 1 else cap_ncores(ncores)
