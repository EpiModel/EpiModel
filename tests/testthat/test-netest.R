...
test_that("differing length non-nested update_dissolution tests", {
  ...
  diss_het <- dissolution_coefs(~offset(edges) + offset(nodematch("age", diff = TRUE)), c(300, 50, 150, 225), 0.001)
  diss_hom <- dissolution_coefs(~offset(edges), 200, 0.001)
  ...
  est_hom_compare <- update_dissolution(est_het, diss_hom, nested_edapprox = FALSE)
  est_het_compare <- update_dissolution(est_hom, diss_het, nested_edapprox = FALSE)
  ...
  expect_equal(est_hom_compare$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges))
  expect_equal(est_het_compare$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("age", diff = TRUE)))
})
...
test_that("non-nested EDA", {
  ...
  diss_het_1 <- dissolution_coefs(~offset(edges) + offset(nodematch("race", diff = TRUE)), c(300, 200, 100, 50, 150, 225), 0.001)
  diss_het_2 <- dissolution_coefs(~offset(edges) + offset(nodematch("age", diff = TRUE)), c(55, 45, 65, 80), 0.001)
  ...
  expect_equal(est_non_1$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("race", diff = TRUE)))
  expect_equal(est_non_2$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("age", diff = TRUE)))

  expect_equal(est_nest_1_non_2$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("age", diff = TRUE)))
  expect_equal(est_non_1_non_2$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("age", diff = TRUE)))
  expect_equal(est_non_2_nest_1$formation, ~edges + nodematch("race", diff = TRUE))
  expect_equal(est_non_2_non_1$formation, ~edges + nodematch("race", diff = TRUE) + offset(edges) + offset(nodematch("race", diff = TRUE)))
})
...
test_that("environment handling in non-nested EDA", {
  ...
  expect_error(netdx_1 <- netdx(netest_1, nsims = 2, nsteps = 5, dynamic = TRUE), "object 'a' not found")
  ...
  expect_error(netest_3 <- update_dissolution(netest_2, coef_diss_2, nested_edapprox = TRUE), "Term options for one or more terms in dissolution model")
  netest_3 <- update_dissolution(netest_2, coef_diss_2, nested_edapprox = FALSE)
  expect_error(netdx_3 <- netdx(netest_3, nsims = 2, nsteps = 5, dynamic = TRUE), "object 'x' not found")
  netest_4 <- update_dissolution(netest_2, coef_diss_2, nested_edapprox = FALSE, from.new = "x")
