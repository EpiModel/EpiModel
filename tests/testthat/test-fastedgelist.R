...
# this requires the tergmLite package from github.  If not installed, check for appropriate error
...
if(!hasTergmLite){
  ...
  control <- control.net(type = "SIR", nsteps = 2, nsims = 1,
                             ...
                             fast.edgelist=TRUE,
                             verbose=FALSE)
  expect_error(netsim(est2, param, init, control), regexp = 'fast_edgelist mode requires installing the tergmLite package')
