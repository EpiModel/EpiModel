...
# this requires the tergmLite package from github.
# If not installed, check for appropriate error

hasTergmLite <- require(tergmLite,quietly = TRUE)

if (!hasTergmLite) {
  # run the test for appropriate error msg
  ...
  init <- init.net(i.num = 10,
                   r.num = 0)
  control <- control.net(type = "SIR", nsteps = 2, nsims = 1,
                             tea.status = FALSE,
                             save.network = TRUE,
                             use.pids = FALSE,
                             fast.edgelist = TRUE,
                             verbose = FALSE)
  expect_error(netsim(est2, param, init, control),
               regexp = 'fast_edgelist mode requires installing the tergmLite package')

} else {

# tergmLite is present, so proceed with the fast_edgelist tests
  ...
  # number of timesteps to run each model;
  # 100 for realistic output, 5 to just make sure they run
  NUMSTEPS <- 5
  ...
  test_that('mode switch works', {
    ...
    control_new <- control.net(type = "SIR",
                               nsteps = NUMSTEPS, nsims = NUMSIMS,
                               tea.status = FALSE,
                               save.network = TRUE,
                               use.pids = FALSE,
                               fast.edgelist = TRUE,
  ...
  test_that('edges+nodematch model works', {
    ...
    coef.diss <- dissolution_coefs(dissolution = ~offset(edges), duration = 20,
                                   d.rate = 0.0021)
    # set iInf coef for offset statistic for nodematch
    #   "never form ties between vertcies with non matching attributes
    coef.form <- -Inf

    # Reestimate the model with new coefficient
    set.seed(1)
    est2 <- netest(nw, formation, target.stats, coef.diss,
                   coef.form = coef.form, verbose = FALSE)
    ...
    plot(simold, y = 'i.num', qnts = 0.5,
         main = paste('i.num comparison for',sims, 'net and edgelist models'))
  ...
  test_that('edges+nodematch(diff=TRUE) model works', {
    ...
    # this seems to run fine in manual testing, but not inside testthat, different environment evaluation?
    formation <- ~edges+offset(nodematch('rolemode',diff=TRUE,keep=2:3))
    target.stats <- 50

    # calculate dissolution coefficient with death rate
    coef.diss <- dissolution_coefs(dissolution = ~offset(edges), duration = 20,
                                   d.rate = 0.0021)
    # set iInf coef for offset statistic for nodematch "never form ties
    #    between vertcies with non matching attributes
    coef.form <- c(-Inf, -Inf)
    ...
    plot(simold, y = 'i.num', qnts = 0.5, main = paste('i.num comparison for',
                                                    sims, 'net and edgelist models'))
    plot(simnew, y = 'i.num', qnts = 0.5, add = TRUE)
    # dev.off()
  ...
  test_that('non-supported models give error',{
    ...
    control_old <- control.net(type = "SIR", nsteps = 2, nsims = sims,
                               ...
                               fast.edgelist = TRUE,
                               verbose = TRUE)
    expect_error(netsim(est2, param, init, control_old),
                 regexp = 'fast_edgelist mode does not know how to update the term')
  })
  ...
  compareNetVsElmodel <- function(est, modelName, param, init, numSteps=100, numSims = 1){
    ...
    # model summary info
    plot(0:10, type = "n", xaxt = "n", yaxt = "n",
         bty = "n", xlab = "", ylab = "", main = 'model params:')
    ...
    plot(simEl, y = 'i.num', qnts = 0.5, add = TRUE, mean.col = 'green' )
    ...
    boxplot(x = list(net = as.numeric(simNoEl$epi$i.num[numSteps,]),
            ...
            main = paste(modelName, 'final i.num distribution,',
                         numSims, 'sims, ', numSteps, ' steps'))
  ...
  test_that('edges+absdiff model works',{
    ...
    param <- param.net(inf.prob = 0.3,
                       rec.rate = 0.02,
                       # need to leave births at zero because have not defined module to give new values
                       b.rate = 0.00,
                       ds.rate = 0.01,
                       di.rate = 0.01,
                       dr.rate = 0.01)
  ...
  plotvital <- function(netsim){
    ...
    plot(seq.int(length(deaths)), deaths, type = 'l',
         ylab = 'deaths and birthds', col = 'red', xlab = 'sim step')
