...
test_that("SI, 1G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1),
                     act.rate = 0.25)
  init <- init.dcm(s.num = 500, i.num = 1)
  control <- control.dcm(type = "SI", nsteps = 10, verbose = FALSE)
  x <- dcm(param, init, control)
...
test_that("SI, 1G, CL: varying act.rate", {
  param <- param.dcm(trans.rate = 0.1,
...
test_that("SI, 1G, CL: varying trans.rate and act.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.8, 0.1),
                     act.rate = seq(0.25, 2, 0.25))
  init <- init.dcm(s.num = 500, i.num = 1)
  control <- control.dcm(type = "SI", nsteps = 10, verbose = FALSE)
  x <- dcm(param, init, control)
...
test_that("SI, 2G, CL: 1 run", {
  param <- param.dcm(trans.rate = 0.2,
...
test_that("SI, 2G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1),
                     act.rate = 0.25, trans.rate.g2 = 0.1, balance = "g1")
...
test_that("SI, 1G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05),
...
test_that("SI, 2G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05),
                     act.rate = 0.25, trans.rate.g2 = 0.1, balance = "g1",
                     b.rate = 1/100, b.rate.g2 = NA, ds.rate = 1/100,
                     ds.rate.g2 = 1/100, di.rate = 1/90, di.rate.g2 = 1/90)
...
test_that("SIR, 1G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05),
...
test_that("SIR, 2G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05),
                     act.rate = 0.25, trans.rate.g2 = 0.1, balance = "g1",
                     rec.rate = 1/100, rec.rate.g2 = 1/100)
  init <- init.dcm(s.num = 500, i.num = 1, r.num = 0,
...
test_that("SIR, 1G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05),
...
test_that("SIR, 2G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.5, 0.05), trans.rate.g2 = 0.1,
                     act.rate = 1, balance = "g1",
                     rec.rate = 1/50, rec.rate.g2 = 1/50,
                     b.rate = 1/100, b.rate.g2 = NA,
                     ds.rate = 1/100, ds.rate.g2 = 1/100,
...
test_that("SIS, 1G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1),
                     act.rate = 0.25, rec.rate = 1/50)
  init <- init.dcm(s.num = 500, i.num = 1)
  control <- control.dcm(type = "SIS", nsteps = 10, verbose = FALSE)
...
test_that("SIS, 2G, CL: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1),
                     act.rate = 0.25, trans.rate.g2 = 0.1, balance = "g1",
                     rec.rate = 1/100, rec.rate.g2 = 1/100)
  init <- init.dcm(s.num = 500, i.num = 1,
...
test_that("SIS, 1G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1),
                     act.rate = 0.5, rec.rate = 1/50,
                     b.rate = 1/100, ds.rate = 1/100, di.rate = 1/90)
  init <- init.dcm(s.num = 500, i.num = 1)
...
test_that("SIS, 2G, OP: 1 run", {
  param <- param.dcm(trans.rate = 0.2,
                     trans.rate.g2 = 0.1, act.rate = 0.5,
...
test_that("SIS, 2G, OP: varying trans.rate", {
  param <- param.dcm(trans.rate = seq(0.1, 0.9, 0.1), trans.rate.g2 = 0.1,
