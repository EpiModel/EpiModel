...
viral_load_logger <- function(dat, at) {
  if (at %% 10 == 0) {
    dat <- with(get_attr_list(dat, c("status", "vl")), {
...
2. `dat <- with(get_attr_list(dat, c("status", "vl")), {` allows to access
   `status` and `vl` directly as variables. In order for `dat` to actually be
   modified, it must be assigned back and also `dat` must be the last call
   inside the `with` expression.
