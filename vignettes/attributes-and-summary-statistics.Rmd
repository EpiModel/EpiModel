...
control <- control.net(
  ...
  tracked.attributes.once = c("race"),
  verbose = FALSE
)
```

Attributes listed in `tracked.attributes` are diff-tracked: at each step,
only nodes whose values changed are recorded. Attributes listed in
`tracked.attributes.once` are recorded only in the initial snapshot and for
newly arriving nodes---they are not compared at each step. Use
`tracked.attributes.once` for attributes that are fixed at entry and never
change during the simulation (e.g., `"race"`, `"birth.year"`).
...
aging_track_module <- function(dat, at) {
  ...
  dat <- set_epi(dat, "mean_age", mean_age)
  dat <- set_epi(dat, "age_change", age_change)
