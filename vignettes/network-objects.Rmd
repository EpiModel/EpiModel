...
## Positional IDs and Unique IDs

EpiModel uses two identification systems for nodes:

- **Positional IDs** (`posit_id`): The index position of a node in attribute vectors. These are used by current edgelists and most attribute accessors. In a model with 100 nodes, positional IDs range from 1 to 100. When nodes depart (die or emigrate), their positional ID slot remains but is marked inactive (`active = 0`). When new nodes arrive, they receive new positional IDs.

- **Unique IDs** (`unique_id`): A globally unique identifier assigned to each node at creation. Unlike positional IDs, unique IDs are never reused. They are used by cumulative edgelists and attribute histories because they can refer to nodes that have already departed.
