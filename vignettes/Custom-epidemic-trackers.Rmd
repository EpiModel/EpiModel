...
the user to pass trackers to the model just before runtime through the `param`
object.
...
2. use `with` and `get_attr_list(dat, needed_attributes)` to work in an
   ...
   the tracker does and simplify the debugging process.
3. define `out` as the calculated outcome, it MUST be a scalar.
4. `return(out)`

# The `tracker.list` parameter

Now that we saw what **trackers** are, we need to tell `EpiModel` to use them.

To do so, we need to declare them as a named list in `param.net`
...
`trackers.net`. This module has to be enabled in `control.net`.
