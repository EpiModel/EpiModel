...
Similarly to the scenarios, the `param.net` function that creates the `param` object for the `netsim` function comes with a `data.frame.params` argument to pass in many parameters at once.

The `data.frame` of parameters to be given as input first 3 columns must be:
1. 'param': the name of the parameter. If it is a multi dimension parameter, prepend the name, with `_1`, `_2`, etc for the position in the vector.
2. 'value': the value for the parameter (or the Nth position if multidimensional)
3. 'type': a character string containing either "numeric", "logical" or "character". The type the value will be cast to.

```{r param_table}
df_params <- tribble(
  ~param, ~value, ~type,
  "hiv.test.rate_1",  "0",            "numeric",
  "hiv.test.rate_2",  "0",            "numeric",
  "hiv.test.rate_3",  "0",            "numeric",
  "prep.require.lnt", "TRUE",         "logical",
  "group_1",          "first group",  "character",
  "group_2",          "second group", "character"
)
param <- param.net(data.frame.params = df_params)
knitr::kable(df_params)
```

Apart from these 3 columns, the `data.frame` can contain any number of other columns. A typical use case would be to have a "details" and "source" columns to document where these parameters come from. As with the scenarios, it is recommended to store these parameters in a CSV or Excel file so they can be easily shared and edited.
