...
Network simulations (`netsim`) use a module-based architecture. Each simulation time step executes a sequence of modules:

1. **Initialization** (`net.mod.init.R`) - Set up initial conditions
2. **Network simulation** (`net.mod.simnet.R`) - Simulate network dynamics via ERGM/STERGM
3. **Infection** (`net.mod.infection.R`) - Disease transmission across edges
4. **Recovery** (`net.mod.recovery.R`) - Disease recovery transitions
5. **Vital dynamics** (`net.mod.vital.R`) - Arrivals and departures
6. **Network update** (`net.mod.nwupdate.R`) - Post-step network bookkeeping
