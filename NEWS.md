...
-   Fix the handling of the `netsim` slots that hold one element per simulation, which the three code paths that touch them did not agree on. `saveout.net()` named `".records"`, a slot that does not exist, so `raw.records` came back unnamed. `get_sims()` subset `epi`, `run`, `network`, `stats$nwstats`, `stats$transmat`, `diss.stats`, and the `save.other` elements, but left `coef.form`, `cumulative.edgelist`, `attr.history`, and `raw.records` at their original length. `merge.netsim()` merged a similar partial list, so `run`, `coef.form`, `cumulative.edgelist`, `attr.history`, and `raw.records` silently kept only the first object's simulations. The consequence for `coef.form` was a wrong result rather than a length mismatch: a restart reads `run[[s]]` and `coef.form[[s]]` positionally, so restarting from `get_sims(x, 2)` used simulation 1's formation coefficients with simulation 2's network state. All three paths now work from one list of per-simulation slots, and `get_sims()` returns simulations in the order requested and renames the `epi` columns along with the other slots. Closes #1059.
-   Fix `merge.netsim()` so that two objects whose controls were built by separate calls to `control.net()` can be merged. The equality check compared `set.control.ergm` and `set.control.tergm` with `identical()`, and those `statnet` control objects carry formulas whose environments differ between calls, so the merge failed with "x and y have different controls" unless the same `control.net()` object had been reused. They are now compared by value, as `monitors` and `nwstats.formula` already were. This affects the manual parallelization workflow that `merge.netsim()` exists to support.
