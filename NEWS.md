...
-   Fix `dedup_cumulative_edgelist()`, which did not merge the spells it is documented to merge. For an edge with spells `[1, 3]` and `[2, 4]` it returned two rows, the second with its `start` rewritten to 1 and its `stop` left at 4, rather than the single merged row `[1, 4]`. More damaging, a spell with an `NA` stop, which marks an edge that was never observed to dissolve, was dropped whenever that edge had another spell, so an active partnership was reported as ended. The function now performs an interval merge per edge: overlapping spells collapse transitively, `NA` stops are treated as open-ended and absorb the spells they overlap, and spells that merely touch are left separate since they do not overlap. Spells are grouped by every column other than `start` and `stop`, so an edgelist carrying a `network` column is merged within each network rather than across networks, and the output is sorted by edge and then `start`. Closes #1063.
